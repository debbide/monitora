...
export async function checkMonitor(monitor: Monitor) {
  ...
  if (
    ...
    await sendTgMessage(monitor.tg_notify_chat_id, msg, {
      reply_markup: {
        inline_keyboard: [[{ text: '🔄 立即重试', callback_data: `retry_scheduled:${monitor.id}` }]]
      }
    })
  }
  ...
  if (!incidents || incidents.length === 0) {
    ...
    if (monitor.check_type === 'komari') {
      ...
      if (consecutiveFailures < 2) {
        console.log(
          `Komari monitor ${monitor.name}: waiting for consecutive failures (${consecutiveFailures}/2)`
        )
        return
      }
    }

    run(`INSERT INTO incidents (monitor_id, started_at, notified) VALUES (?, ?, 0)`, [
      monitor.id,
