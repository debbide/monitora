...
添加监控 -> 选择类型 Scheduled Webhook。

调度模式：选择 随机区间，填入 10 - 12，单位选择 Days。
...
让服务器的脚本定时往群里发 "Server A is Online"。
...
逻辑：面板如果没有在规定时间内（间隔 x 2）收到包含 Server A 和 Online 的消息，就标记为 Down。
...
欢迎提交 Issue 或 Pull Request。更多详情请查看 CONTRIBUTING.md
。

这就是完整的 README 文件，已经包括了 Webhook 被动接收 配置部分。如果你有任何进一步修改的需求或其他问题，欢迎告诉我！
